...
// Fixture replacing the removed FIXED_SCHEDULES constant.
// Davoted's schedule (arkbar + loveBeach) used only by buildAvailabilityMap tests below.
const DAVOTED_FIXED_SCHEDULES = {
  'Davoted': {
    arkbar: {
...
describe('Availability logic (buildAvailabilityMap)', () => {
  ...
  test('injects FIXED_SCHEDULES DJ (Davoted) on correct weekdays', () => {
    ...
    const map = buildAvailabilityMap({
      ...
      submittedNames: new Set(),
      month: MARCH_2026,
      fixedSchedules: DAVOTED_FIXED_SCHEDULES,
    });
...
describe('fixedSchedules from DB', () => {
  ...
  // Constant-shaped: same as FIXED_SCHEDULES['Davoted'] (integer keys) — uses test fixture
  const CONST_FIXED_SCHEDULES = DAVOTED_FIXED_SCHEDULES;
