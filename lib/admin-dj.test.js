...
// ─────────────────────────────────────────────────────────────────────────────
// updateFixedSchedules
// ─────────────────────────────────────────────────────────────────────────────

describe('updateFixedSchedules', () => {
  let updateFixedSchedules;
  ...
  beforeEach(() => {
    ...
    mockInvalidateCaches.mockClear();
    const supabase = makeMockSupabase();
    ({ updateFixedSchedules } = createAdminDJHandlers(supabase, mockBcrypt, mockInvalidateCaches));
  });

  test('returns { success: true } on valid input', async () => {
    const fixed_schedules = { arkbar: { '1': ['14:00\u201315:00'] }, loveBeach: { '2': ['20:00\u201321:00'] } };
    const result = await updateFixedSchedules({ id: 'uuid-1', fixed_schedules });
    expect(result.success).toBe(true);
  });

  test('returns { success: false, error, status: 400 } when id is missing', async () => {
    const result = await updateFixedSchedules({ fixed_schedules: { arkbar: {} } });
    expect(result.success).toBe(false);
    expect(result.error).toBe('id is required');
    expect(result.status).toBe(400);
  });

  test('returns { success: false, error, status: 400 } when fixed_schedules is missing', async () => {
    const result = await updateFixedSchedules({ id: 'uuid-1' });
    expect(result.success).toBe(false);
    expect(result.error).toBe('fixed_schedules is required');
    expect(result.status).toBe(400);
  });

  test('rejects unknown venue keys with { success: false, status: 400 }', async () => {
    const result = await updateFixedSchedules({ id: 'uuid-1', fixed_schedules: { unknownVenue: { '1': [] } } });
    expect(result.success).toBe(false);
    expect(result.status).toBe(400);
    expect(result.error).toContain('Invalid venue key');
  });

  test('rejects mixed valid/invalid venue keys', async () => {
    const result = await updateFixedSchedules({ id: 'uuid-1', fixed_schedules: { arkbar: {}, badVenue: {} } });
    expect(result.success).toBe(false);
    expect(result.status).toBe(400);
    expect(result.error).toContain('badVenue');
  });

  test('calls supabase.from("djs").update({ fixed_schedules }).eq("id", id) on valid input', async () => {
    const fixed_schedules = { arkbar: { '1': ['14:00\u201315:00'] } };
    await updateFixedSchedules({ id: 'uuid-1', fixed_schedules });
    expect(mockCalls.from[0]).toBe('djs');
    expect(mockCalls.update[0].payload).toEqual({ fixed_schedules });
    expect(mockCalls.update[0].table).toBe('djs');
  });

  test('calls invalidateCaches("djs") on success', async () => {
    const fixed_schedules = { loveBeach: { '5': ['20:00\u201321:00'] } };
    await updateFixedSchedules({ id: 'uuid-1', fixed_schedules });
    expect(mockInvalidateCaches).toHaveBeenCalledWith('djs');
  });
  ...
  test('returns { success: false } on supabase error', async () => {
    mockUpdateError = { message: 'DB error' };
    const result = await updateFixedSchedules({ id: 'uuid-1', fixed_schedules: { arkbar: {} } });
    expect(result.success).toBe(false);
    expect(result.error).toBeDefined();
  });

  test('accepts empty object (clear all fixed schedules)', async () => {
    const result = await updateFixedSchedules({ id: 'uuid-1', fixed_schedules: {} });
    expect(result.success).toBe(true);
  });
});
