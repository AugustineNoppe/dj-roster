 ...
 * @returns {{ listDJs, addDJ, editDJ, resetPin, clearLockout, updateRecurringAvailability }}
 */
function createAdminDJHandlers(supabase, bcrypt, invalidateCaches) {
  ...
  /**
   * Unlock a DJ's submission for a given month by deleting their
   * dj_availability and dj_submissions rows for that month.
   *
   * @param {{ id: string, month: string }} params
   * @returns {Promise<{ success: boolean, message?: string, error?: string, status?: number }>}
   */
  async function unlockSubmission({ id, month } = {}) {
    if (!id) return { success: false, error: 'id is required', status: 400 };
    if (!month || !String(month).trim()) return { success: false, error: 'month is required', status: 400 };

    try {
      // Look up the DJ name from the id
      const { data: djRow, error: lookupErr } = await supabase
        .from('djs')
        .select('name')
        .eq('id', id)
        .single();
      ...
      if (lookupErr || !djRow) {
        return { success: false, error: 'DJ not found', status: 404 };
      }

      const djName = djRow.name.trim();
      const trimmedMonth = String(month).trim();

      // Delete availability rows and submission row for this DJ + month
      const [availResult, subResult] = await Promise.all([
        supabase.from('dj_availability').delete().eq('name', djName).eq('month', trimmedMonth),
        supabase.from('dj_submissions').delete().eq('name', djName).eq('month', trimmedMonth),
      ]);

      if (availResult.error) throw new Error(availResult.error.message);
      if (subResult.error) throw new Error(subResult.error.message);

      invalidateCaches('availability', { month: trimmedMonth });
      return { success: true, message: `Submission unlocked for ${djName} — ${trimmedMonth}` };
    } catch (err) {
      console.error('[unlockSubmission] unexpected error:', err.message || err);
      return { success: false, error: 'Failed to unlock submission' };
    }
  }

  return { listDJs, addDJ, editDJ, resetPin, clearLockout, updateRecurringAvailability, unlockSubmission };
}
