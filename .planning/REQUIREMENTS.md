...
- [ ] **DB-01**: Single `djs` table created with columns: id, name, pin_hash, rate, type, active, venues, recurring_availability (JSONB), fixed_schedules (JSONB), failed_attempts, locked_until
- [ ] **DB-02**: Migration script populates `djs` from dj_rates + dj_pins, deduplicating en-dash/hyphen name variants
- [ ] **DB-03**: Migration seeds recurring_availability JSONB from FIXED_AVAILABILITY constants and fixed_schedules JSONB from FIXED_SCHEDULES constants
